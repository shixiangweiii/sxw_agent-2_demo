...
| 成功的 Assistant 语义历史 | Authority | `run_events.ASSISTANT_MESSAGE_COMMITTED`；Finalize Transaction | Conversation history、Prompt | 把失败 Run 的 partial delta、EOF 或 request-local history 纳入后续历史 |
...
| 外部业务对象真实状态 | Authority | 外部 Tool 系统 | Runtime 的 `external_object_ref` 与 reconcile 结果 | Runtime 声称其 SQLite 可原子裁决外部对象；把 runtime.db 与 demo side-effect DB 当一个事务 |
...
| Evidence 完整集合 | Artifact | committed EvidenceSet Artifact；Retrieval/Tool Broker | citation、模型 preview | filename、显示序号 `[n]` 或 request-local CitationInjector 当稳定证据身份 |
...
| Release 清单 | Authority | immutable `release_manifests`；Worker 注册器 | digest、运维查询 | 原地修改同一 release；运行中静默切换 Prompt/Tool/Skill policy |
...
Checkpoint 是带 revision、release/schema 和 CAS 的可恢复业务事实。History 是从 committed 用户消息、仅成功提交的 assistant message 与当前 checkpoint 编译出的模型输入投影。失败 partial delta 不进入 History。
...
Runtime 只裁决自己掌握的 `PREPARED/DISPATCHED/COMMITTED/FAILED/UNKNOWN/...` 证据；外部工单、消息或 demo task 的真实状态仍由外部系统裁决。二者通过稳定 idempotency key 和 `external_object_ref` 关联，不伪造跨库原子事务。
