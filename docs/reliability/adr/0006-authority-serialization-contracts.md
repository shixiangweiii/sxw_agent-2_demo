...
1. 六份 v1 Schema 必须验证真实权威对象的 `model_dump(mode="json")` 或真实 Broker 规范化产物，而不是描述一个并不存在的汇总 DTO。
...
6. Tool Broker 在写 EvidenceSet Artifact 前规范化完整上下文和 provenance：run/activity/tool execution、principal、dataset/scope/query、RetrievalStatus/degraded reasons，以及每条 Evidence 的 document/index version、content hash、page/span、scope/query identity。模型仍只接收有界 hits preview。
