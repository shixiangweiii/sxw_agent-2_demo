...
# 降方差：--repeat 3（关键 suite 跑 N 次取多数/均值，强烈建议≥2）；不重跑 LLM 只重评分：$PY -m eval.harness.rescore --out eval/reports/<ts>
```

> 首版报告已产出（`eval/reports/20260629-090605/`，44 次运行 0 硬门违规 0 过度路由），并做过 A/B prompt 回归（`baseline-r3` vs `improved-r3`，解读见 `eval/reports/AB-prompt-v1.md`）。**关键结论：同一 prompt 改动对两代引擎效果相反**——plan_execute 净增益、agent_loop 净回退，故按引擎**选择性上线**。这是「两引擎不只是编排不同、行为会分叉」的实证依据，改 prompt/工具集时务必两引擎都跑。
...
- **claude-skill 沙箱**（`agent/claude_skill/`）：技能包 `SKILL.md` 作为**子代理在沙箱中执行**；沙箱 provider 抽象（`LocalSandbox` 可跑 / `AgentBay` 云桩）。纯本地能力，不依赖任何下游。
...
- **claude-skill（沙箱）**：在 `agent/claude_skill/skills_data/<id>/SKILL.md` 写 frontmatter(name/description) + 指令体，自动被加载。
